...
"dojo/dom-attr",
 ...
 "dojo/_base/sniff"
  ...
  /*
   * 
   * Phil added these to get metadata to display
   * '<img src="agsjsTOCPLPMod/2.09/src/agsjs/dijit/metadata.png" alt="" data-dojo-attach-point="metaNode"  data-dojo-attach-event="onclick:_onMetaClick" />' +
   * insert
   *  data-dojo-attach-event="onmouseover:_onMetaMouseOver" into did not end up doing this.
   *  '<span data-dojo-attach-point="labelNode" data-dojo-attach-event="onmouseover:_onMetaMouseOver">' +
   *  
   *  destroy if it is a rootLayer in:
   *  _createRootLayerNode add
   *  domConstruct.destroy(this.metaNode);
   *  _createLegendNode add
   *  domConstruct.destroy(this.metaNode);
   *  
   *  add dojo/tooltip to define statement
   *  add reference to tooltip in _createServiceLayerNode
   *  
   *  needed to add some stuf to toc constructor to get layerlist 
   *  this.layerList = params.layerList;
   *  
   *  also needed to add some stuff to pass layer list to noed
   *  tocLayerList:this.layerList
   *  tocLayerList2:this.layerList
   *  
   *  var myShortDescription = "";
      for (var layerIdx = 0; layerIdx < this.rootLayerTOC.tocLayerList.length; layerIdx++){
          if (serviceLayer.name === this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("name")[0].childNodes[0].nodeValue){
              myShortDescription = this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("shortDescription")[0].childNodes[0].nodeValue;
          }
      }
      if (myShortDescription === ""){
          myShortDescription = serviceLayer.name;
      }
      var myTooltip = new Tooltip({connectId: this.labelNode, label: myShortDescription});
  ...
  var _TOCNode = declare([_Widget, _Templated],{
    ...
    _createServiceLayerNode: function(serviceLayer){
      ...
      //alert(this.layerList.length);
      //alert(this.rootLayerTOC.tocLayerList2.length + " : creating toc");
      var myShortDescription = "";
//      for (var layerIdx = 0; layerIdx < this.rootLayerTOC.tocLayerList.length; layerIdx++){
//          //alert(this.labelNode.innerHTML + " : " + this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("name")[0].childNodes[0].nodeValue)
//          if (serviceLayer.name === this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("name")[0].childNodes[0].nodeValue){
//              myShortDescription = this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("shortDescription")[0].childNodes[0].nodeValue;
//          }
//      }
      for (var layerIdx = 0; layerIdx < this.rootLayerTOC.tocLayerList2.length; layerIdx++){
          if (serviceLayer.name === this.rootLayerTOC.tocLayerList2[layerIdx].name){
              myShortDescription = this.rootLayerTOC.tocLayerList2[layerIdx].shortDescription;
          }
      }
    ...
    _onMetaClick: function (evt){
      
      
      var myShortDescription = "";
      var myMetaURL = "";
      var myOtherURL = "";
      var myLongDescription = "";
      var myContent = "";
      for (var layerIdx = 0; layerIdx < this.rootLayerTOC.tocLayerList2.length; layerIdx++){
          if (String(this.labelNode.innerHTML).replace(/&amp;/g, "&") === this.rootLayerTOC.tocLayerList2[layerIdx].name){
              myMetaURL = this.rootLayerTOC.tocLayerList2[layerIdx].metaURL;
              myOtherURL = this.rootLayerTOC.tocLayerList2[layerIdx].detailsURL;
              myShortDescription = this.rootLayerTOC.tocLayerList2[layerIdx].shortDescription;
              myLongDescription = this.rootLayerTOC.tocLayerList2[layerIdx].longDescription;
          }
          
      }
      //alert(myMetaURL);
//      for (var layerIdx = 0; layerIdx < this.rootLayerTOC.tocLayerList.length; layerIdx++){
//          //var myStr = String(this.labelNode.innerHTML).replace(/&amp;/g, "&");
//          //alert(myStr);
//          
//          //alert(this.labelNode.innerHTML + " : " + this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("name")[0].childNodes[0].nodeValue)
//          if (String(this.labelNode.innerHTML).replace(/&amp;/g, "&") === this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("name")[0].childNodes[0].nodeValue){
//              myMetaURL = this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("metaURL")[0].childNodes[0].nodeValue;
//              myOtherURL = this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("detailsURL")[0].childNodes[0].nodeValue;
//              myShortDescription = this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("shortDescription")[0].childNodes[0].nodeValue;
//              myLongDescription = this.rootLayerTOC.tocLayerList[layerIdx].getElementsByTagName("longDescription")[0].childNodes[0].nodeValue;
//          }
//      }
      //alert(myLongDescription);
      myContent = "<div style='top:0px;float:right;'><input type='button' value='x'/></div>";
      myContent += "<div>";
      myContent += "<p>";
      myContent += myLongDescription;
      myContent += "</p><p>";
      myContent += myMetaURL;
      myContent += "</p>";
      myContent += "</p><p>";
      myContent += myOtherURL;
      myContent += "</p>";
      myContent += "</div>";
      
      //alert(myContent);
      var myTooltipDialog = new TooltipDialog({content: myContent, onClick:function(){popup.close(myTooltipDialog);},style: "width 250px;"});
      popup.open({popup: myTooltipDialog, around: this.metaNode});
      //window.open(myMetaURL);
 ...
 var _RootLayerTOC = declare([_Widget], {
  ...
  var TOC = declare("agsjs.dijit.TOC", [_Widget, Evented],{
    ...
    constructor: function(params, srcNodeRef){
      ...
      this.layerListUrl = params.layerListUrl;
      lang.mixin(this, params);
    },
    // extension point
    postCreate: function(){
...
//      }
//      
//      xmlhttp.open("GET",this.layerListUrl,false);
//      xmlhttp.send();
//      
//      var xmlDoc = xmlhttp.responseXML;
//      
//      var myLayers = xmlDoc.getElementsByTagName("layer");
//       
//      var layerName = myLayers[0].getElementsByTagName("name")[0].childNodes[0].nodeValue;
      var myLayers = [];
      var myLayers2 = layerMetadata;
      //alert(myLayers2.length);
      this._rootLayerTOCs = [];
      //alert(this.layerList.length);
      for (var i = 0, c = this.layerInfos.length; i < c; i++) {
		...
		var rootLayerTOC = new _RootLayerTOC({
          config: info,
          tocWidget: this,
          tocLayerList:myLayers,
          tocLayerList2:myLayers2
        });
